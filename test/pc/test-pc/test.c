...
#if 0
    uint16 seg_id = x3d_segfaceid_seg(caster.hit_face);
    uint16 face = x3d_segfaceid_face(caster.hit_face);

    X3D_UncompressedSegment* seg = x3d_segmentmanager_load(seg_id);
    X3D_UncompressedSegmentFace* faces = x3d_uncompressedsegment_get_faces(seg);

    X3D_Plane* plane = &faces[face].plane;
    X3D_Mat3x3 mat;

    x3d_plane_guess_orientation(plane, &mat, &caster.hit_pos);
...
void engine_test(void) {
  ...
  x3d_prism3d_construct(prism, base_v, 800,  600, angle);
