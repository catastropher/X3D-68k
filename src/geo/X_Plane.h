...
static inline int x_plane_point_distance(const X_Plane* plane, const X_Vec3* point)
{
    return x_vec3_dot(&plane->normal, point) + plane->d;
}

static inline _Bool x_plane_point_is_on_normal_facing_side(const X_Plane* plane, X_Vec3* point)
{
