...
static bool send_data(Connection* con, char* data, int size, unsigned char cmd)
{
...
static bool connection_is_ready(Connection* con)
{
    return con->state == CONNECTION_REQUEST_CONNECT;
}

static bool socket_open(X_Socket* socket, const char* address)
{
...
static bool open_connection(Connection* con, int port)
{
     // FIXME: what should happen if the port is already open?
    con->cable = ticables_handle_new(CABLE_USB, (CablePort)port);
    if(!con->cable)
    {
...
static bool send_packet(X_Socket* socket, X_Packet* packet)
{
     if(packet->size >= 240)
    ...
    SendPacket* sendPacket = (SendPacket*)malloc(sizeof(SendPacket));
    
    sendPacket->size = packet->size + 1;
    sendPacket->data = (char*)malloc(sendPacket->size);
...
static void recv_packet(X_Socket* socket)
{
    ...
    packet->size = size - 2;
    packet->type = (X_PacketType)buf[0];
...
static bool wait_for_connect_packet(Connection* con)
{
...
static void* connection_thread(void* data)
{
    ...
    while(1)
    {
        volatile ConnectionState state = (ConnectionState)connection_get_state(con);
...
static bool match_address(const char* address)
{
    return 1;
}
...
static bool get_connect_request(X_ConnectRequest* request)
{
    for(int i = 0; i < MAX_CONNECTIONS; ++i)
    {
...
static void* listen_for_connections_thread(void* data)
{
    while(1)
    {
        ...
        for(int i = 0; i < MAX_CONNECTIONS; ++i)
        {
            ...
            CalcModel model;
            if(ticalcs_probe(CABLE_USB, (CablePort)i, &model, 1) == 0)
            {
